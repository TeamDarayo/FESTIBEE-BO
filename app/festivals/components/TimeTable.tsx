...
export default function TimeTable({ 
  ...
  const handleSaveArtists = async () => {
    ...
    try {
      ...
      // 삭제된 아티스트 처리
      const artistsToDelete = existingArtists.filter(existing => 
        !newArtists.some(newArtist => newArtist.artistId === existing.artistId)
      );
      ...
      for (const artist of artistsToDelete) {
        if (artist.timetableArtistId) {
          await deleteTimeTableArtist(selectedTimeTable.id!, artist.timetableArtistId);
        }
      }
